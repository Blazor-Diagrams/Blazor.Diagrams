...
public partial class NavigatorWidget : IDisposable
{
    ...
    private void Diagram_GroupAdded(GroupModel group)
    {
        group.Changed += Refresh;
    }

    private void Diagram_GroupRemoved(GroupModel group)
    {
        group.Changed -= Refresh;
    }
    ...
    private void Refresh(Model? _)
    {
        if (BlazorDiagram != null)
        {
            ...
            (var fullSizeWidth, var fullSizeHeight) = GetFullSize(nodesMaxX, nodesMaxY);
            AdjustFullSizeWithNodesRect(nodesMinX, nodesMinY, ref fullSizeWidth, ref fullSizeHeight);

            NodePositionAdjustment = new Point(nodesMinX < 0 ? Math.Abs(nodesMinX) : 0,
                nodesMinY < 0 ? Math.Abs(nodesMinY) : 0);
        ...
        ref double fullSizeHeight)
    {
        ...
        if (nodesMinX < 0)
        {
            ...
            else if (BlazorDiagram.Pan.X <= 0) fullSizeWidth += Math.Abs(nodesMinX);
        }
        ...
        if (nodesMinY < 0)
        {
            ...
            else if (BlazorDiagram.Pan.Y <= 0) fullSizeHeight += Math.Abs(nodesMinY);
        }
    }
