...
namespace Blazor.Diagrams.Core.Positions.Resizing
{
    public abstract class ResizerProvider : IPositionProvider
    {
        ...
        virtual public void ResizeNode(double deltaX, double deltaY)
        {
            ...
            var width = _originalSize.Width + (ShouldAddTotalMovedX ? _totalMovedX : -_totalMovedX);
            var height = _originalSize.Height + (ShouldAddTotalMovedY ? _totalMovedY : -_totalMovedY);

            var positionX = _originalPosition.X + (ShouldChangeXPositionOnResize ? _totalMovedX : 0);
            var positionY = _originalPosition.Y + (ShouldChangeYPositionOnResize ? _totalMovedY : 0);
        ...
        virtual public void OnPanChanged(double deltaX, double deltaY, double clientX, double clientY)
        {
            if (_nodeModel is null) return;
